...
{
    class DefeatLabel : Label
    {
        static Texture2D texture;
        protected int timeSpawned;
        protected ParticleEmitter particleEmit;

        public DefeatLabel(Vector2 aLocation, Game1 aContext)
        {
            position = aLocation;
            texture = aContext.Content.Load<Texture2D>("defeat_label");
        ...
        public override void Update(GameTime gameTime, Game1 context)
        {
            ...
            {
                context.stateManager.FadeOut(StateEnum.DeathState);
                //context.stateManager.ChangeState(StateEnum.DeathState);
            }
